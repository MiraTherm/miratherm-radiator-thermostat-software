...
void Driver_Test(osMessageQueueId_t storage2system_event_queue,
  ...
  const TickType_t config_wait_ticks =
      pdMS_TO_TICKS(5000U); /* 5 second timeout */

  osStatus_t status = osMessageQueueGet(storage2system_event_queue, &event,
                                        NULL, config_wait_ticks);
  printf("osMessageQueueGet for storage2system_event_queue returned status=%d, "
         "event=%d\n",
         status, event);
  ...
  for (size_t i = 0; i < 3; ++i) {
    ...
    lv_obj_align(buttons[i].btn, LV_ALIGN_BOTTOM_LEFT,
                 margin + i * (button_width + gap), -margin);
  ...
  const size_t button_event_count =
      sizeof(button_event_types) / sizeof(button_event_types[0]);
  ...
  for (;;) {
    ...
    const bool event_ready =
        (osMessageQueueGet(input2vp_event_queue, &event, NULL,
                           event_wait_ticks) == osOK);
        ...
        if (event.type == EVT_CTRL_WHEEL_DELTA) {
          ...
          if (idx < button_event_count) {
            ...
            if (pressed != buttons[idx].active) {
              ...
              const lv_color_t bg =
                  pressed ? lv_color_white() : lv_color_black();
              const lv_color_t txt =
                  pressed ? lv_color_black() : lv_color_white();
