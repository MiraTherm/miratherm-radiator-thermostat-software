...
WaitingView_t* WaitingView_Init(const char *message, int16_t y_ofs)
{
    ...
    lv_label_set_text(view->label, message ? message : "");
    lv_obj_align(view->label, LV_ALIGN_CENTER, 0, y_ofs);
...
{
    if (view) {
        if (view->screen) lv_obj_del(view->screen);
        free(view);
    }
}
...
{
    if (view && view->label && message) {
        if (lv_port_lock()) {
            lv_label_set_text(view->label, message);
            lv_port_unlock();
        }
    }
}
