...
MenuView_t *MenuView_Init(const char *options) {
  ...
  lv_obj_set_size(view->list, LV_HOR_RES - 3,
                  LV_VER_RES - 17); /* Leave space for hints */
  ...
  /* Add buttons */
  /* Note: We are hardcoding buttons here because lv_list works by adding
     buttons, and parsing the newline separated string every time is
     inefficient. Ideally, the presenter should pass an array or we just know
     what the menu is. Given the context, we know the items. */
  ...
  lv_obj_set_style_bg_color(view->btn_schedule, lv_color_white(),
                            LV_STATE_FOCUS_KEY);
  lv_obj_set_style_text_color(view->btn_schedule, lv_color_black(),
                              LV_STATE_FOCUS_KEY);
  ...
  lv_obj_set_style_bg_color(view->btn_offset, lv_color_white(),
                            LV_STATE_FOCUS_KEY);
  lv_obj_set_style_text_color(view->btn_offset, lv_color_black(),
                              LV_STATE_FOCUS_KEY);
  ...
  lv_obj_set_style_bg_color(view->btn_factory_rst, lv_color_white(),
                            LV_STATE_FOCUS_KEY);
  lv_obj_set_style_text_color(view->btn_factory_rst, lv_color_black(),
                              LV_STATE_FOCUS_KEY);
...
void MenuView_Hide(MenuView_t *view) { (void)view; }
