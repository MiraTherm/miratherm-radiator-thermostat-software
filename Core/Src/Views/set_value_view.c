...
SetValueView_t *SetValueView_Init(const char *title, const char *unit,
                                  const char *options) {
  ...
  lv_obj_set_style_text_color(view->roller_value, lv_color_black(),
                              LV_PART_SELECTED);
...
void SetValueView_Render(SetValueView_t *view,
  ...
  if (view->last_options_str != data->options_str) {
    if (data->options_str) {
      lv_roller_set_options(view->roller_value, data->options_str,
  ...
  if (view->last_selected_index != data->selected_index) {
    lv_roller_set_selected(view->roller_value, data->selected_index,
                           LV_ANIM_OFF);
