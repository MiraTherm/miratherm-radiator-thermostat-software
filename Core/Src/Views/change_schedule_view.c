...
ChangeScheduleView_t *ChangeScheduleView_Init(void) {
  ChangeScheduleView_t *view =
      (ChangeScheduleView_t *)malloc(sizeof(ChangeScheduleView_t));
  if (!view)
    return NULL;

  /* Initialize sub-views. They are created but not shown yet. */
  /* Note: We initialize them with default/placeholder values, they will be
   * updated by presenter */

  view->bool_view = SetBoolView_Init("Edit schedule?", "Yes", "No", false);
...
void ChangeScheduleView_Deinit(ChangeScheduleView_t *view) {
  if (view) {
    if (view->bool_view)
      SetBoolView_Deinit(view->bool_view);
    if (view->value_view)
      SetValueView_Deinit(view->value_view);
    if (view->time_slot_view)
      SetTimeSlotView_Deinit(view->time_slot_view);
    free(view);
  }
}
...
SetTimeSlotView_t *
ChangeScheduleView_GetTimeSlotView(ChangeScheduleView_t *view) {
