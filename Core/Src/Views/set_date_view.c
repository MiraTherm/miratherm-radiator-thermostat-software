...
SetDateView_t *SetDateView_Init(const char *title,
  ...
  lv_obj_set_style_text_color(view->roller_year, lv_color_black(),
                              LV_PART_SELECTED);
  ...
  lv_obj_set_style_text_color(view->roller_month, lv_color_black(),
                              LV_PART_SELECTED);
  ...
  lv_obj_set_style_text_color(view->roller_day, lv_color_black(),
                              LV_PART_SELECTED);
...
static void update_date_roller_borders(SetDateView_t *view,
  ...
  if (view->last_active_field != active_field) {
    ...
    lv_obj_t *active_roller = (active_field == 0)   ? view->roller_year
                              : (active_field == 1) ? view->roller_month
                                                    : view->roller_day;
