...
{
    SetValuePresenter_t *presenter = (SetValuePresenter_t *)malloc(sizeof(SetValuePresenter_t));
    if (!presenter) return NULL;

    presenter->view = view;
    presenter->data.selected_index = initial_index;
    presenter->data.options_str = NULL; /* Managed by view or external setter if needed, here we just use index */
...
void SetValuePresenter_HandleEvent(SetValuePresenter_t *presenter, const Input2VPEvent_t *event)
{
    ...
    if (event->type == EVT_CTRL_WHEEL_DELTA)
    {
        ...
        if (new_index > presenter->max_index) new_index = presenter->max_index;
...
void SetValuePresenter_SetSelectedIndex(SetValuePresenter_t *presenter, uint16_t index)
{
    if (presenter)
    {
        if (index > presenter->max_index) index = presenter->max_index;
