...
BoostPresenter_t* BoostPresenter_Init(BoostView_t *view, SystemContextAccessTypeDef *system_context)
{
    if (!view || !system_context)
        return NULL;

    BoostPresenter_t *presenter = (BoostPresenter_t *)malloc(sizeof(BoostPresenter_t));
    if (!presenter)
        return NULL;
        ...
        switch (event->type)
        {
            case EVT_CENTRAL_BTN:
                ...
                if (presenter->system_context)
                {
                    ...
                    {
                        SystemMode_t previous_mode = presenter->system_context->data.mode_before_boost;
...
void BoostPresenter_Run(BoostPresenter_t *presenter, uint32_t current_tick)
{
    ...
    if (osMutexAcquire(presenter->system_context->mutex, 10) == osOK)
    {
        uint32_t elapsed_ticks = osKernelGetTickCount() - presenter->system_context->data.boost_begin_time;
