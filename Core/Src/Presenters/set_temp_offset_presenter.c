...
SetTempOffsetPresenter_t* SetTempOffsetPresenter_Init(SetValueView_t *view, ConfigAccessTypeDef *config_access)
{
    ...
    SetTempOffsetPresenter_t *presenter = (SetTempOffsetPresenter_t *)malloc(sizeof(SetTempOffsetPresenter_t));
    if (!presenter)
        return NULL;
    ...
    SetValueView_SetOptions(view, "-2.0\n-1.5\n-1.0\n-0.5\n0.0\n+0.5\n+1.0\n+1.5\n+2.0");
...
void SetTempOffsetPresenter_HandleEvent(SetTempOffsetPresenter_t *presenter, const Input2VPEvent_t *event)
{
    ...
    if (SetValuePresenter_IsComplete(presenter->generic_presenter) && !presenter->is_complete)
    {
        /* Save value */
        uint16_t index = SetValuePresenter_GetSelectedIndex(presenter->generic_presenter);
