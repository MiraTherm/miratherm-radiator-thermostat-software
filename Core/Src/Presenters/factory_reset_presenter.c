...
{
    FactoryResetPresenter_t *presenter = (FactoryResetPresenter_t *)malloc(sizeof(FactoryResetPresenter_t));
    if (!presenter)
        return NULL;
    ...
    presenter->confirm_view = SetBoolView_Init("Factory reset?", "Yes", "No", true);
    if (presenter->confirm_view)
    {
        SetBoolView_Show(presenter->confirm_view);
        presenter->confirm_presenter = SetBoolPresenter_Init(presenter->confirm_view);
    }
...
void FactoryResetPresenter_HandleEvent(FactoryResetPresenter_t *presenter, const Input2VPEvent_t *event)
{
    ...
    if (presenter->state == FR_STATE_CONFIRM)
    {
        if (presenter->confirm_presenter)
        {
            ...
            {
                const SetBool_ViewModelData_t *data = SetBoolPresenter_GetData(presenter->confirm_presenter);
                if (data && data->value)
                {
                    ...
                    /* Init Progress */
                    presenter->progress_view = LoadingView_Init("Factory Reset", LV_ALIGN_LEFT_MID, 10);
                    if (presenter->progress_view)
                    {
                        presenter->progress_presenter = LoadingPresenter_Init(presenter->progress_view);
                    }
