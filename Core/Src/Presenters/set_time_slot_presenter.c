...
static const uint8_t MINUTES_COUNT =
    12; /* 5-minute resolution: 0, 5, 10, ..., 55 minutes */
static const uint8_t MINUTE_STEP = 5;

SetTimeSlotPresenter_t *SetTimeSlotPresenter_Init(SetTimeSlotView_t *view) {
  SetTimeSlotPresenter_t *presenter =
      (SetTimeSlotPresenter_t *)malloc(sizeof(SetTimeSlotPresenter_t));
  if (!presenter)
    return NULL;
...
void SetTimeSlotPresenter_HandleEvent(SetTimeSlotPresenter_t *presenter,
  ...
  if (event->type == EVT_CTRL_WHEEL_DELTA) {
    ...
    if (presenter->data.active_field == 0 &&
        !presenter->data.start_time_locked) {
      int16_t val = (int16_t)presenter->data.start_hour + delta;
      if (val < 0)
        val = HOURS_COUNT - 1;
      else if (val >= HOURS_COUNT)
        val = 0;
      ...
      state_changed = true;
               ...
               !presenter->data.start_time_locked) {
      int16_t val =
          (int16_t)(presenter->data.start_minute / MINUTE_STEP) + delta;
      if (val < 0)
        val = MINUTES_COUNT - 1;
      else if (val >= MINUTES_COUNT)
        val = 0;
      presenter->data.start_minute = (uint8_t)(val * MINUTE_STEP);
      state_changed = true;
    } else if (presenter->data.active_field == 2 &&
               !presenter->data.end_time_locked) {
      ...
      else if (val == presenter->data.start_hour &&
               presenter->data.end_minute <= presenter->data.start_minute) {
      ...
      state_changed = true;
    } else if (presenter->data.active_field == 3 &&
               !presenter->data.end_time_locked) {
      ...
      /* Prevent end time from being less than or equal to start time */
      /* If we're in the same hour as start time, don't allow end_minute <=
       * start_minute */
     ...
     */
    /* Requirement says: "On the first time slot begin time (always 00:00)
     * should be blocked." */
    /* This implies we shouldn't even edit it. */

    /* Simple logic: increment active field until we find an unlocked one or
     * finish */
    ...
    */
    ...
    if (presenter->data.active_field == 2 && presenter->data.end_time_locked) {
      /* If we are at field 2 and end time is locked, pressing OK should
       * complete */
    ...
    if (presenter->data.active_field > 0) {
      ...
      /* If we are at field 2 and go back, and start time is locked (fields 0,1
         locked), we should probably not go there? But the "back_hint"
         requirement usually means going back to previous screen if at first
         field. Here we are talking about fields.
      */

      if (prev_field == 1 && presenter->data.start_time_locked) {
        /* Start time locked means 0 and 1 are locked. So we can't go back to 1.
         */
        /* We are at 2 (EndHH). If we press back, we should probably exit the
           presenter (handled by router) OR stay at 2. But usually MODE button
           at first editable field is handled by router to go back to previous
           step. So here we only handle if we can go back to a field.
        */
      } else {
